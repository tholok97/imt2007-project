...
\textbf{DHCP, ARP and IP spoofing attacks}: To prevent rogue DHCP servers and DHCP starvation we will use DHCP Snooping. While our upstream ports will be set to trusted, access port will have a limit to how many DHCP request it can send per second. However, it may take some time for the DHCP Snooping to finish the binding table. In addition we will use DAI (Dynamic ARP Inspection) to prevent ARP spoofing and poisoning. Then only valid
