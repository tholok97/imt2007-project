...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \end{enumerate}
    \item Passwords should follow NIST recommendations~\cite{nistpassword}, meaning we prefer long easily memorable passwords to short difficult ones.
...
\\
\\
The hardening of branch layer 2 is equal to our HQ implementation.
...
We have worked hard and reached a result we can confidently say we are pleased with. This should not overshadow the fact that we had apostasy and poor communication in the beginning of the project.
\\
\\
...
\\
\\
We must admit our group collaboration was not very good in the beginning. However, when we moved from using Google Docs to using Github with Overleaf the group effort got massive boost. The introduction of version control gave us a central tool to work around, while still allowing each member to work the way they wanted.
\\
\\
